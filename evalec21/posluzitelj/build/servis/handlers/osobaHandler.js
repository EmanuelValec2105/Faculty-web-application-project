...
class OsobaHandler {
    ...
    async dohvatiSve(req, res) {
        ...
        try {
            const osobe = await this.baza.izvrsiUpit(`SELECT * FROM osoba LIMIT ${brojPoStranici} OFFSET ${offset} `);
    ...
    async dodaj(req, res) {
        try {
            ...
            const postojecaOsoba = await this.baza.izvrsiUpit("SELECT * FROM osoba WHERE tmdb_id = ?", [tmdb_id]);
            ...
            const rezultat = await this.baza.izvrsiIzmjenu(`INSERT INTO osoba (tmdb_id, ime, poznat_po, popularnost, slika) 
                 VALUES (?, ?, ?, ?, ?)`, [
                tmdb_id,
                tmdbOsoba.name,
            ...
            if (slike.profiles) {
                for (const profil of slike.profiles) {
                    await this.baza.izvrsiIzmjenu("INSERT INTO galerija_slika (osoba_id, slika_url) VALUES (?, ?)", [rezultat.lastID, profil.file_path]);
                }
            }
            ...
            if (filmovi.cast) {
                for (const film of filmovi.cast) {
                    ...
                    let filmId;
                    const postojeciFilm = await this.baza.izvrsiUpit("SELECT id FROM film WHERE tmdb_id = ?", [film.id]);
                    if (postojeciFilm.length === 0) {
                        // Dodaj novi film
                        const rezultatFilm = await this.baza.izvrsiIzmjenu(`INSERT INTO film (tmdb_id, jezik, originalni_naslov, naslov, 
                    ...
                    await this.baza.izvrsiIzmjenu("INSERT INTO osoba_film (osoba_id, film_id, uloga) VALUES (?, ?, ?)", [rezultat.lastID, filmId, film.character]);
                }
            }
    ...
    async dohvatiJednu(req, res) {
        try {
            const tmdbId = Number(req.params.id); // Konverzija u broj
            console.log("Primljeni TMDB ID:", tmdbId);
            const [osoba] = await this.baza.izvrsiUpit("SELECT * FROM osoba WHERE tmdb_id = ?", [tmdbId]);
            osoba.galerija = await this.baza.izvrsiUpit("SELECT * FROM galerija_slika WHERE osoba_id = ?", [osoba.id]);
            osoba.filmovi = await this.baza.izvrsiUpit("SELECT * FROM osoba_film INNER JOIN film on film.id = osoba_film.film_id WHERE osoba_film.osoba_id = ? LIMIT 20", [osoba.id]);
    ...
    async obrisi(req, res) {
        try {
            ...
            const filmovi = await this.baza.izvrsiUpit("SELECT * FROM osoba_film where osoba_id = ?", [osoba.id]);
            await this.baza.izvrsiIzmjenu("DELETE FROM osoba_film WHERE osoba_id = ?", [osoba.id]);
            for (let film of filmovi) {
                const imaJos = (await this.baza.izvrsiUpit("SELECT * FROM osoba_film where film_id = ?", [film.id])).length > 0;
                if (imaJos == false) {
...
exports.OsobaHandler = OsobaHandler;
